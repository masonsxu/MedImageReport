...
from utils import Checkpoint, Logger, Smoother, Step, to_device


def compute_batch(model, source, targets, verbose=False, optional_ret=None):
    if optional_ret is None:
        optional_ret = []
...
def array2str(arr):
    ...
    for i in range(len(arr)):
        if arr[i] in [conf['pad_id'], conf['eos_id']]:
            break
...
def get_model():
    return TranslationModel(
        ...
        conf['output_l'],
        conf['n_token'],
        encoder_layer=conf['n_layer'],
        decoder_layer=conf['n_layer'],
    )
...
def train():
    ...
    train_loader = DataLoader(
        train_data, batch_size=conf['batch'], shuffle=True, num_workers=1, drop_last=False
    )
    valid_loader = DataLoader(
        valid_data, batch_size=conf['valid_batch'], shuffle=True, num_workers=1, drop_last=False
    )
    ...
    for epoch in range(start_epoch, conf['n_epoch']):
        ...
        for (source, targets) in tqdm(train_loader):
            ...
            for x in losses:
                if x[:5] == 'loss_':
                    loss += eval(f'conf["w_{x[5:]}"]') * losses[x]
            losses['loss'] = loss
...
def inference(model_file, data_file):
    ...
    test_loader = DataLoader(
        test_data, batch_size=conf['valid_batch'], shuffle=False, num_workers=1, drop_last=False
    )
