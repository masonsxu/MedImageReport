...
def to_device(data, device):
    if isinstance(data, torch.Tensor):
        ...
        data = [to_device(item, device) for item in data]
...
class Smoother:
    ...
    def value(self, key=None, mean=True):
        if mean:
            if key is None:
                return {
                    key: self.sum[key] / min(len(self.num[key]), self.window) for key in self.num
                }
    ...
    def log(self, *args, end='\n'):
        for x in args:
            if isinstance(x, dict):
                for y in x:
                    self.fp.write(f'{str(y)}:{str(x[y])} ')
            else:
                self.fp.write(f'{str(x)} ')
        self.fp.write(end)
        self.cnt += 1
    ...
    def update(self, file_path, logger=None, **kwargs):
        ...
        for metric in metrics:
            if metrics[metric] is None:
                continue
            if (
                metric not in self.contents['best_metrics']
                or metrics[metric] > self.contents['best_metrics'][metric]
            ):
                self.contents['best_metrics'][metric] = metrics[metric]
                torch.save(self._get_contents(), f'{file_path[:-3]}_{metric}.pt')
